...
var FileDiff = require('./filediff');
var helpers = require('../common/helpers');

var TreeDiff = module.exports = function TreeDiff(repo, prevTreeSha, thisTreeSha, path) {
  ...
  var changesToStat = function(changes) {
    ...
    helpers.each(changes, function(change) {
  ...
  that.incWaiting = function incWaiting() {
    that.waitingFor += 1;
  };
  
  that.decWaiting = function decWaiting() {
    that.waitingFor -= 1;
    that.finishIfDone();
  };
  
  that.finishIfDone = function finishIfDone() {
  ...
  that.compareBlobEntry = function compareBlobEntry(prevBlobEntry, thisBlobEntry) {
  ...
  that.compareTreeEntry = function(prevTreeEntry, thisTreeEntry) {
    ...
    if (!prevTreeEntry) {
      generator.incWaiting()
      var subTreeDiff = new TreeDiff(repo, null, thisTreeEntry.sha)
      ...
      generator.incWaiting()
      var subTreeDiff = new TreeDiff(repo, prevTreeEntry.sha, null)
      subTreeDiff.generateChanges(function(subChanges) {
        generator.changes = generator.changes.concat(subChanges)
        generator.decWaiting()
      ...
      generator.incWaiting()
      var subTreeDiff = new TreeDiff(repo, prevTreeEntry.sha, thisTreeEntry.sha)
      subTreeDiff.generateChanges(function(subChanges) {
        generator.changes = generator.changes.concat(subChanges)
        generator.decWaiting()
      }, generator.path + thisTreeEntry.name + "/")
  ...
  that.toString = function(callback) {
