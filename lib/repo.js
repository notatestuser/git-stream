...
Repo.prototype.receivePack = function(opts) {

  if (!opts.refs) {
    throw new Error('No refs provided');
  }
  ...
  //
  // The returned response is a stream describing 
  // each ref and its current value. The stream MUST be 
  // sorted by name according to the C locale ordering.
  //
...
Repo.prototype.config = {
  ...
  that.getLooseObject = function(sha, callback) {
    ...
    fs.readFile(objectPath, function(err, data) {
      ...
      var uncompressedData = new Buffer(RawDeflate.inflate(Git.bytesToString(deflated)).toString())
